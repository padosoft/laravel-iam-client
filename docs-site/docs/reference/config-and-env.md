...
| `http.allow_insecure` | `IAM_CLIENT_ALLOW_INSECURE` | bool | `false` | **IAM-39/39b**: allow credentials over `http://`. Default `false` = fail-closed: a non-`https` `oauth_url` **or** `base_url` (except loopback `localhost`/`127.0.0.1`/`::1`) sends no secret/assertion/Bearer — the decision call itself denies. Dev only |
